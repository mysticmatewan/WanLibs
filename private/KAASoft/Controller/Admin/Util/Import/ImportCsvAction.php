<?php
    ...
    class ImportCsvAction extends AdminActionBase {
        ...
        private function processBookAuthors($book, $authorArray) {
            ...
            foreach (explode($this->multipleValuesDelimiter,
                ...
                if (!ValidationHelper::isEmpty($authorName)) {
                    ...
                    $existingAuthor = $this->getExistingAuthor($author);
                    if ($existingAuthor == null) {
                        $existingAuthor = $author;
                        $this->authors[] = $existingAuthor;
                    }

                    $bookAuthors[] = $existingAuthor;
                }
            }
        ...
        private function processBookGenres($book, $genreArray) {
            ...
            foreach (explode($this->multipleValuesDelimiter,
                ...
                if (!ValidationHelper::isEmpty($genreName)) {
                    ...
                    $existingGenre = $this->getExistingGenre($genre);
                    if ($existingGenre == null) {
                        $existingGenre = $genre;
                        $this->genres[] = $existingGenre;
                    }

                    $bookGenres[] = $existingGenre;
                }
            }
        ...
        private function processBookStores($book, $storeArray) {
            ...
            foreach (explode($this->multipleValuesDelimiter,
                ...
                if (!ValidationHelper::isEmpty($storeName)) {
                    ...
                    $existingStore = $this->getExistingStore($store);
                    if ($existingStore == null) {
                        $existingStore = $store;
                        $this->stores[] = $existingStore;
                    }

                    $bookStores[] = $existingStore;
                }
            }
        ...
        private function processBookLocations($book, $locationArray) {
            ...
            foreach (explode($this->multipleValuesDelimiter,
                ...
                if (!ValidationHelper::isEmpty($locationName)) {
                    ...
                    $existingLocation = $this->getExistingLocation($location);
                    if ($existingLocation == null) {
                        $existingLocation = $location;
                        $this->locations[] = $existingLocation;
                    }

                    $bookLocations[] = $existingLocation;
                }
            }
        ...
        private function processBookPublisher($book, $publisherArray) {
            //ControllerBase::getLogger()->info("Start.");
            $publisher = Publisher::getObjectInstance($publisherArray);
            if (!ValidationHelper::isEmpty($publisher->getName())) {
                $existingPublisher = $this->getExistingPublisher($publisher);
                if ($existingPublisher == null) {
                    $existingPublisher = $publisher;
                    $this->publishers[] = $existingPublisher;
                }
        ...
        private function processBookSeries($book, $seriesArray) {
            //ControllerBase::getLogger()->info("Start.");
            $series = Series::getObjectInstance($seriesArray);
            if (!ValidationHelper::isEmpty($series->getName())) {
                $existingSeries = $this->getExistingSeries($series);
         ...
         * @return null|Author
         */
        private function getExistingAuthor($author) {
            foreach ($this->authors as $arrayAuthor) {
                if ($author->compareTo($arrayAuthor)) {
                    return $arrayAuthor;
                }
            }
         ...
         * @return Genre|null
         */
        private function getExistingGenre($genre) {
            foreach ($this->genres as $arrayGenre) {
                if ($genre->compareTo($arrayGenre)) {
                    return $arrayGenre;
                }
            }
         ...
         */
        private function getExistingPublisher($publisher) {
            foreach ($this->publishers as $arrayPublisher) {
                if ($publisher->compareTo($arrayPublisher)) {
         ...
         * @return null|Series
         */
        private function getExistingSeries($series) {
            foreach ($this->series as $arraySeries) {
                if ($series->compareTo($arraySeries)) {
                    return $arraySeries;
                }
            }
         ...
         * @return null|Store
         */
        private function getExistingStore($store) {
            foreach ($this->stores as $arrayStore) {
                if ($store->compareTo($arrayStore)) {
                    return $arrayStore;
                }
            }
         ...
         */
        private function getExistingLocation($location) {
            foreach ($this->locations as $arrayLocation) {
                if ($location->compareTo($arrayLocation)) {
