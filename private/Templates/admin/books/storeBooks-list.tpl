        ...
        <tr>
            ...
            <th class="d-none header-bulk-actions">
                <div class="btn-group" role="group">
                    ...
                    {*<button class="btn btn-success btn-sm"><i class="fa fa-pencil"></i> {t}Edit Books{/t}</button>*}
                </div>
            </th>
            <th class="header-cell">{t}Book{/t}</th>
            <th style="width: 130px;" class="text-center"><span class="header-cell">{t}Location{/t}</span></th>
        ...
        {if isset($books) and $books != null}
            {foreach from=$books item=book name=book}
                <tr>
                    ...
                    <td class="text-center">
                        {if $book !== null and $book->getLocations() !== null and is_array($book->getLocations())}
                            {foreach from=$book->getLocations() item=location name=location}
                                <a href="{$routes->getRouteString("locationBooksView",["locationId"=>$location->getId()])}">{$location->getName()}</a>{if $smarty.foreach.location.last}{else},{/if}
                            {/foreach}
                        {/if}
                    </td>
                    <td class="text-center">
                    ...
                    <td class="text-center">
                        <a href="{$routes->getRouteString("bookEdit",["bookId"=>$book->getId()])}" class="btn btn-outline-info btn-sm mr-1" data-container="body" data-toggle="tooltip" title="{t}Edit{/t}"><i class="fa fa-pencil"></i></a>
                        <a href="{$routes->getRouteString("bookClone",["bookId"=>$book->getId()])}" class="btn btn-outline-info btn-sm mr-1" data-container="body" data-toggle="tooltip" title="{t}Clone{/t}"><i class="fa fa-clone"></i></a>
