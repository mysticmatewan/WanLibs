            ...
            {foreach from=$posts item=post name=post}
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{$routes->getRouteString("postViewPublic",["postUrl"=>$post->getUrl()])}" class="btn btn-outline-info btn-sm {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}View{/t}"><i class="fa fa-eye"></i></a>
                        <a href="{$routes->getRouteString("postEdit",["postId"=>$post->getId()])}" class="btn btn-outline-info btn-sm {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}Edit{/t}"><i class="fa fa-pencil"></i></a>
                        {if $post->getId() != 0}
                        <div class="dropdown d-inline" data-trigger="hover" data-toggle="tooltip" title="{t}Delete{/t}">
                            ...
                            <ul class="dropdown-menu {if $activeLanguage->isRTL()}dropdown-menu-left{else}dropdown-menu-right{/if}">
                                ...
                                <li class="text-center">
                                    <button class="btn btn-outline-secondary delete-post" data-url="{$routes->getRouteString("postDelete",["postId"=>$post->getId()])}">
