            ...
            {foreach from=$postPages item=page name=page}
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{$routes->getRouteString("pageViewPublic",["pageUrl"=>$page->getUrl()])}" class="btn btn-outline-info btn-sm {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}View{/t}"><i class="fa fa-eye"></i></a>
                        <a href="{$routes->getRouteString("pageEdit",["pageId"=>$page->getId()])}" class="btn btn-outline-info btn-sm {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}Edit{/t}"><i class="fa fa-pencil"></i></a>
                        {if $page->getId() != 0}
                            <div class="dropdown d-inline" data-trigger="hover" data-toggle="tooltip" title="{t}Delete{/t}">
                                ...
                                <ul class="dropdown-menu {if $activeLanguage->isRTL()}dropdown-menu-left{else}dropdown-menu-right{/if}">
                                    ...
                                    <li class="text-center">
                                        <button class="btn btn-outline-secondary delete-page" data-url="{$routes->getRouteString("pageDelete",["pageId"=>$page->getId()])}">
