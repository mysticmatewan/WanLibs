...
<div class="row">
    ...
    {if isset($books) and $books != null}
        {foreach from=$books item=book name=book}
            <div class="{$size_grid|default:"col-sm-6 col-md-4 col-lg-3"}">
                <div class="card book card-no-border">
                    {if $book->getCover() != null}
                        <div class="cover">
                            <a href="{$routes->getRouteString("bookViewPublic",["bookId"=>$book->getId()])}"><img src="{$book->getCover()->getWebPath('small')}" alt="{$book->getTitle()}" class="card-img-top"></a>
                        </div>
                    {else}
                        <div class="cover">
