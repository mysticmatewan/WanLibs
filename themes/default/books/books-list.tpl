...
<div class="row book-list">
    ...
    {if $smarty.post.books_row != null}
        {assign var="books_row" value=$smarty.post.books_row}
    {/if}
    {if $smarty.post.size_grid != null}
        {assign var="size_grid" value=$smarty.post.size_grid}
    {/if}

    {if isset($books) and $books != null}
        {foreach from=$books item=book name=book}
            <div class="{$size_grid|default:"col-sm-12 col-md-4 col-lg-4"} {$books_row|default:"book-2-row"}">
                <div class="row book" itemscope itemtype="http://schema.org/Book">
                    ...
                    <div class="book-cover col-lg-4 col-4">
                        {if $book->getCover()}
                            <a href="{$routes->getRouteString("bookViewPublic",["bookId"=>$book->getId()])}"><img src="{$book->getCover()->getWebPath('small')}" alt="{$book->getTitle()}" class="img-fluid"></a>
                        {else}
                            <a href="{$routes->getRouteString("bookViewPublic",["bookId"=>$book->getId()])}"><img src="{$siteViewOptions->getOptionValue("noBookImageFilePath")}" alt="{$book->getTitle()}" class="img-fluid"></a>
                        {/if}
                    </div>
                    <div class="book-info col-lg-8 col-8">
                        <div class="book-title">
                            <a href="{$routes->getRouteString("bookViewPublic",["bookId"=>$book->getId()])}">{$book->getTitle()}</a>
                        </div>
                        <div class="book-attr">
                            {if $book->getPublishingYear() != null}<span class="book-publishing-year">{$book->getPublishingYear()}{if $book->getAuthors() !== null}, {/if}</span>{/if}
                            {if $book->getAuthors() !== null and is_array($book->getAuthors()) and count($book->getAuthors()) > 0}
                                {assign var="authors" value=$book->getAuthors()}<span class="book-author" itemprop="author">{$authors[0]->getLastName()}{$authors[0]->getFirstName()}</span>
                            {/if}
                        </div>
                        <div class="book-rating">
                            {include 'books/rating.tpl' rating=$book->getRating() readOnly=true}
